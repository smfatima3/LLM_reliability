...
CONFIG = {
    ...
    'alert_thresholds': { 'r_metric': 0.55 } # R is now just normalized ΔL, so we alert when it's high
}

class ReliabilityMonitor:
    def __init__(self, config):
        self.config = config; self.loss_history = deque(maxlen=100); self.delta_l_history = deque(maxlen=1000)
    ...
    def calculate_r_metric(self, lambda_val, sigma_sq_val, delta_l_val):
        ...
        # A high, positive drift is a sign of instability.
        delta_l_norm = self._normalize(delta_l_val, self.delta_l_history)
        # We call it 'r_metric' to keep the analysis script consistent.
        # The alert logic in check_alerts() is now "if delta_l_norm > threshold".
...
class TrainingSimulator:
    ...
    def _get_simulated_grad_norms(self): return [] # Not used in this ablation
    ...
    def run(self):
        ...
        for step in range(1, self.config['total_training_steps'] + 1):
            ...
            if step % self.config['eval_every_n_steps'] == 0:
                ...
                r_metric_data = self.monitor.calculate_r_metric(0, 0, self.monitor.calculate_delta_l(self.state['validation_loss']))
                self._check_alerts(r_metric_data['r_metric'])
                log_data = {'step': step, 'event': 'METRICS', 'training_loss': self.state['training_loss'], 'validation_loss': self.state['validation_loss'], 'grad_norm_mean': 0, 'lambda': 0, 'sigma_sq': 0, 'delta_l': self.monitor.delta_l_history[-1], **r_metric_data, 'alerts': self.alerts}
                self._log(log_data)
    ...
    def _check_alerts(self, r_metric):
        # Alert if normalized drift is HIGH
