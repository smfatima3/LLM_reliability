...
# --- Configuration ---
CONFIG = {
    'experiment_id': 'final_run_delta_l',
    'model_type': 'GPT-2',
    # We are now officially defining our metric based on the most reliable signal
    'METRIC_MODE': 'DELTA_L_ONLY', 
    'num_workers': 8,
    'total_training_steps': 5000,
    'eval_every_n_steps': 100,
    'fault_injection': { 'type': 'NODE_FAILURE', 'trigger_step': 2500, 'params': {} },
    # This threshold is now applied to the normalized ΔL score
...
class ReliabilityMonitor:
    def __init__(self, config):
        self.config = config
        self.metric_mode = config.get('METRIC_MODE', 'DELTA_L_ONLY')
    ...
    def calculate_r_metric(self, delta_l_val):
        # The R metric is now defined as the normalized loss drift.
        # A high value indicates high instability.
...
class TrainingSimulator:
    ...
    def _setup_logger(self):
        log_dir = f"logs_{self.config.get('METRIC_MODE', 'final')}"
    ...
    def run(self):
        ...
        for step in range(1, self.config['total_training_steps'] + 1):
            ...
            if step % self.config['eval_every_n_steps'] == 0:
                ...
                r_metric_data = self.monitor.calculate_r_metric(delta_l_val)
                self._check_alerts(r_metric_data['r_metric'])
                log_data = {'step': step, 'event': 'METRICS', 'delta_l': delta_l_val, **r_metric_data, 'alerts': self.alerts}
                self._log(log_data)
